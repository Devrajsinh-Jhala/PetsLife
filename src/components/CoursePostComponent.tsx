        ...
        {posts.map((post) => (
          ...
          >
            <div className="flex flex-col border border-secondary p-2 rounded-lg cursor-pointer group">
                ...
                <div className="absolute bottom-0 w-full bg-opacity-20 bg-black backdrop-blur-lg rounded drop-shadow-lg text-white p-5  flex justify-between">
                  ...
                  <div className="flex flex-col md:flex-row gap-y-2 md:gap-x-3 items-center">
                    {post.categories.map((category, i) => (
                      <div
                        key={i}
                        className="dark:bg-zinc-800 bg-slate-100 text-zinc-800 text-[10px] text-center dark:text-white p-1 rounded-full font-semibold"
                      >
                        <p>{category.title}</p>
                      </div>
                    ))}
              ...
              <p className="line-clamp-1 mt-1 text-sm">{post.description}</p>
            </div>
          </ClientSideRoute>
        ))}
      </div>
